...
tags: ["論文筆記", "NeRF", "Human Reconstruction"]
categories: ["論文筆記"]
description: 本篇筆記為筆者在閱讀 HumanNeRF (CVPR 2022) 這篇論文時的隨記。該研究提出了一種方法，用於從單眼視角的影片（monocular video）中，為移動中的人物生成自由視角的渲染結果。這項技術試圖解決在僅有單一、變動視角輸入的情況下，重建人物在任意新視角下的外觀（包含姿態、衣物等細節）的挑戰。筆記將整理 HumanNeRF 的核心方法，包括如何使用標準體積表示（canonical volume）、分解運動場（motion field decomposition），以及姿態修正（pose correction）等技術來實現其目標。
math: true
---
...
![overview](https://cdn.rxchi1d.me/inktrace-files/Paper_Survey/2023-03-02-HumanNeRF/overview.png)
_Fig.2. Overview. 我們的方法以視頻幀為輸入，並針對標準外觀進行優化，將其呈現為連續場，同時也為從觀察空間到標準空間的運動場進行映射。運動場被拆分為骨架的剛性和非剛性運動，分別以離散網格和連續場的形式呈現。我們還使用現有的身體姿態估計器來初始化身體姿態，進一步進行微調，以獲得更好的對齊效果。我們在觀察空間的體積渲染和輸入圖像之間施加了一個損失，以引導優化朝著解決方案的方向進行。_
{}
...
$$
F_o(\text{x}, \text{p}) = F_c(T(\text{x}, \text{p})), \quad (1)
$$

> where,
> 
> - $F_c : \text{X} \rightarrow (c, \sigma)$ maps position $\text{x}$ to color $\text{c}$ and density $\sigma$。
> - $T : (\text{x}_o, \text{p}) \rightarrow \text{X}_c$ defines a motion field mapping points from observed space back to canonical space, guided by observed pose
> - $\text{p} = (J, \Omega)$: observed pose
...
$$
T(\text{x}, \text{p}) = T_{\text{skel}}(\text{x}, \text{p}) + T_{\text{NR}}(T_{\text{skel}}(\text{x}, \text{p}), \text{p}), \quad (2)
$$

> where,
> 
> - $T_{\text{skel}}$ represents skeleton-driven deformation, essentially inverse (volumetric) linear-blend skinning
> - $T_{\text{NR}}$ starts from the skeleton-driven deformation and produces an offset $\Delta \text{x}$ to it

對於野外場景，作者使用現成的 3D body+camera pose estimator (SPIN)。

因為不準確的 pose estimation，作者也使用了一個 pose correction function $P_{\text{pose}}(\text{p})$ 來更好的解釋 observation，並改善 skeleton-driven deformation。 作者將 Eq.2 中的 $T_{\text{skel}}(\text{x}, \text{p})$ 以 $T_{\text{skel}}(\text{x}, P_{\text{pose}}(\text{p}))$ 取代。

#### Canonical volume

![canonical_space](https://cdn.rxchi1d.me/inktrace-files/Paper_Survey/2023-03-02-HumanNeRF/canonical_space.png)
_Canonical Space_

作者利用一個 MLP 將 canonical volume $F_c$ 表示成一個 continuous field：

$$
F_c(\text{x}) = \text{MLP}_{\theta_c}(\gamma(\text{x})), \quad (3)
...
![skeletal_motion](https://cdn.rxchi1d.me/inktrace-files/Paper_Survey/2023-03-02-HumanNeRF/skeletal_motion.png)
_Skeletal Motion_
...
$$
T_{\text{skel}}(\text{x}, \text{p}) = \sum_{i=1}^{K} w^i_o(\text{x}) (R_i \text{x} + t_i), \quad (4)
$$

> where,
> 
- $w^i_o$ is the blend weight for the $i$-th bone
- $R_i, t_i$ are the rotation and translation, respectively, that map the bone’s coordinates from observation to canonical space.  $R_i$ and $t_i$ can be explicitly computed from $\text{p}$ (see supplementary).

關於 skeletal motion 的點映射，詳細內容寫在補充資料的 **「A. Derivation of Motion Bases」**

首先定義 body pose $\text{p} = (J, \Omega)$，其中 $J = \{j_i\}$ 包含 $K$ 個關節位置，$\Omega = \{ \omega_i \}$ 為使用 axis-angle representations $\in \mathcal{}$$\mathcal{so}(3)$ 表示的 local joint rotations。

給定預先定義的 canonical pose $\text{p}_c = ( J^c, \Omega^c )$ 與 observed pose $\text{p} = (J, \Omega)$，對於 body part $k$ 的 observation-to-canonical transformation $M$ 為 Eq.16：

$$
M_k(\text{p}_c, \text{p}) = \prod_{i \in \tau(k)} \begin{bmatrix}
\exp(\omega^c_i) & j^c_i \\
0 & 1 
\end{bmatrix} \left\{ \prod_{i \in \tau(k)} \begin{bmatrix}
\exp(\omega_i) & j_i \\
0 & 1 
\end{bmatrix} \right\}^{-1} \quad (16)
...
最後， 對於 body part $k$ 的 rotation $R_k$ 與 translation $t_k$ 可以通過 Eq.17 從 $M_k$ 中提取：
...
在實作上，作者通過儲存一組體積  $\{ w_c^i ({\text{x})}\}$ 作為 $K$ 個 blending weight 來求解定義在 canonical space 中的 $w_c^i$，從中得到的 observation weights 是：

$$
w^i_o(\text{x}) = \frac{w^i_c(R_i \text{x} + t_i)}{\sum_{k=1}^{K} w^k_c(R_k \text{x} + t_k)}, \quad (5)
$$

在 canonical space 中求解一組的 weight volumes $\{ w_c^i ({\text{x})}\}$ 可以達到更好的泛化效果，而不是 $N$ 組 $\{ w_o^i ({\text{x})}\}$ ，因為它能夠避免 overfitting 的問題。

作者將一組 $\{ w_o^i ({\text{x})}\}$ 打包成一個具有 $K$ 通道的 volume $W_c(\text{x})$。不通過 MLP 而是直接使用顯式 volume representation 的原因有二：

1. Eq.5 中顯示，當要計算每個 $w_o^i ({\text{x})}$ 時需要 $K$ 個 MLP，這樣對於優化很沒效率 ($K$ = 24 in our work)
2. 通過三線性插值重新取樣的具有有限解析度的顯式體積提供了平滑性，這可以幫助後期的優化規範化。

實作上，並不會直接解出 $W_c$，而是通過一個 CNN ，並輸入一個 random (constant) latent code $\text{z}$ 來得到：

$$
W_c(\text{x}) = \text{CNN}_{\theta_{\text{skel}}}(\text{x}; \text{z}), \quad (6)
$$

作者同時多加了一個通道（background class），因此 $W_c$ 有 $K+1$ 個通道。接著作者在 CNN 的輸出使用 channel-wise softmax，來強制跨通道的統一劃分。

在 Eq.5  中，分母 ($\sum_{k=1}^{K} w^k_c(R_k \text{x} + t_k)$) 可以看作近似物體部分的 likelihood $f(\text{x})$。如果 $f(\text{x})$ 趨近於零，則表示這個點可能在 free space 中。這個概念會在後面 volume rendering 的部分使用到。

#### Non-rigid motion

![non_rigid_motion](https://cdn.rxchi1d.me/inktrace-files/Paper_Survey/2023-03-02-HumanNeRF/non_rigid_motion.png)
_non_rigid_motion.png_

作者將 non-rigid motion $T_{\text{NR}}$ 表示成 skeleton-driven motion 的一個 offset $\Delta \text{x}$：

$$
\Delta \text{x}(\text{x}, \text{p}) = T_{\text{NR}}(T_{\text{skel}}(\text{x}, \text{p}), \text{p})
$$

為了捕捉更多的細節，作者使用 MLP 表示 $T_{\text{NR}}$ (Eq.7)：

$$
T_{\text{NR}}(\text{x}, \text{p}) = \text{MLP}_{\theta_{\text{NR}}}(\gamma(\text{x}); \Omega), \quad (7)
$$

#### Pose correction

從圖像估計的 body pose 可能不準確，因此作者通過 Eq.8 來更新 (修正) pose：

$$
P_{\text{pose}}(\text{p}) = (J, \Delta_\Omega(\text{p}) \otimes \Omega),
$$

過程中作者固定 joints $J$，並優化相關的 joint angles。$\Delta_\Omega = (\Delta \omega_0, \dots, \Delta \omega_K)$ 被添加到 $\Omega$ 中來更新 rotation vectors。

作者發現，不是直接優化 $\Delta_\Omega$，而是通過 MLP 根據 $\Omega$ 產生 $\Delta_\Omega$ (Eq.9)，可以更快收斂：

$$
\Delta_\Omega(\text{p}) = \text{MLP}_{\theta_{\text{pose}}}(\Omega) \quad (9)
$$

經過 pose correction，最終的 warping function 可寫作 Eq.10：

$$
T(\text{x}, \text{p}) = T_{\text{skel}}(\text{x}, P_{\text{pose}}(\text{p})) + T_{\text{NR}}(T_{\text{skel}}(\text{x}, P_{\text{pose}}(\text{p})), \text{p}) \quad (10)
$$

![pose_correction.png](https://cdn.rxchi1d.me/inktrace-files/Paper_Survey/2023-03-02-HumanNeRF/pose_correction.png)
_Pose Correction_
...
給定 input frames $\{I_1, I_2, \ldots, I_N \}$, body poses $\{\text{p}_1, \text{p}_2, \ldots, \text{p}_N \}$ 與 cameras $\{\text{e}_1, \text{e}_2, \ldots, \text{e}_N \}$，本篇模型的優化目標為 Eq.11，也就是最小化所有網路參數 $\Theta = \{\theta_c, \theta_{\text{skel}}, \theta_{\text{NR}}, \theta_{\text{pose}} \}$ 的 loss。

$$
\underset{\Theta}{\text{minimize}} \sum_{i=1}^{N} \mathcal{L}\{\Gamma [F_c(T(\text{x}, \text{p}_i)), \text{e}_i], I_i\}. \quad (11)
...
每個射線的期望顏色 $C(\text{r})$ 會通過 volume rendering (Eq.12) 求得：

$$
\begin{align*}
C(r) &= \sum_{i=1}^{D} \left( \prod_{j=1}^{i-1} (1 - \alpha_j) \right) \alpha_i c(\text{x}_i) \quad (12) \\
\alpha_i &= 1 - \exp(-\sigma(\text{x}_i) \Delta t_i)
\end{align*}
$$
...
當 approximate foreground probability $f(\text{x})$ 很小時，作者進一步擴增 $\alpha_i$ 的定義如 Eq.13 所示：

$$
\alpha_i = f(\text{x}_i)(1 - \exp(-\sigma(\text{x}_i) \Delta t_i)) \quad (13)
$$
...
當作者使用 Eq.11 來優化網路時發現，**skeleton-driven 與 non-rigid motions 在優化過程中並沒有完全拆解** (受試者的一部分骨架運動由非剛性運動場建模 )。這是因為非剛性運動對輸入圖像的過度擬合。因此，在渲染未見過的視角時，品質會降低。

作者通過調整優化過程來解決這個問題。

具體來說，作者在優化開始時禁用非剛性運動，然後在後續的過程中以由粗到細的方式逐步重新引入它們 。對於非剛性運動的 MLP，作者應用了 truncated Hann window 到其 position encoding 的 frequency bands 上，以防止對數據的過擬合，並在優化過程中逐步增大窗口的大小。跟隨 Park et al. [47] 的方法，為 position encoding 的每個 frequency band 定義了權重 (Eq.15)：

$$
\tau(t) = L \frac{\max(0, t - T_s)}{T_e - T_s} \quad (15)
$$
...
- Design 1 (Eq.18)
    ...
    $$
    T(\text{x}) = T_{\text{skel}}(\text{x}) + T_{\text{NR}}(\text{x}) \quad (18)
    $$
    
- Design 2 (Eq.19)
    
    $$
    T(\text{x}) = T_{\text{skel}}(\text{x} + T_{\text{NR}}(\text{x})) \quad (19)
...
- Design 3 (Eq.20)
    ...
    $$
    T(\text{x}) = T_{\text{skel}}(\text{x}) + T_{\text{NR}}(T_{\text{skel}}(\text{x})) \quad (20)
    $$
