...
此外，若進一步引入外部群眾協作圖資（如 OSM 的 `name:zh`），還會遭遇更荒謬的「無來源機器翻譯」污染——例如峇里島的 `Sidemen` 曾被機械翻譯成「伴奏者」（把英文樂手術語丟進翻譯器的產物），`Lubuk Pakam` 被誤配為澳洲的「朗塞斯頓」。這迫使專案在引進多來源交叉驗證時，必須嚴格執行「逐字比對」：OSM 字串必須在 Wikidata 同名實體中有完全相符的中文標籤才放行，只要去字尾後有任何衝突就全數剔除回退原文。
...
> [!NOTE]
> 這個教訓也直接落實在印尼郡級（kecamatan）譯名的篩選中：專案不再依賴脆弱的 `P31` 類別白名單（那會誤殺 `P31=town` 的「烏布」），而是全面採用原文 label 全等比對搭配 P131 遞移驗證，自然過濾掉機關、車站與語言族群實體（如 `Banjar → 班查語`）。
...
修正上游對所有使用者都有效；對照表只對這個專案有效，而且要長期維護。南韓的例子在[上一篇](/posts/engineering/immich-geodata-tech-03-strategies/)提過：原本為了修補 Wikidata 繁中標籤各類靜默錯誤（如誤選區廳、簡繁錯字、舊制未更）而規劃的七條人工對照表，在改用韓文維基的漢字表記作為名稱來源之後，降到了零條。
