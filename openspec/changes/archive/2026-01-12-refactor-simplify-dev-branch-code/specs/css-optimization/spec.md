...
- **WHEN** 撰寫 categories 中 SVG icon 的選擇器
- **THEN** 應使用 `[data-taxonomy="categories"] article h2 svg` (3 層)
- **THEN** 不應使用 `[data-taxonomy="categories"] section.flex.flex-wrap article > h2 > a > span > svg` (7 層)
...
- **WHEN** 在 categories.css 定義 dark mode 樣式
- **THEN** `backdrop-filter` 應從 light mode 繼承
- **THEN** dark mode 只需覆蓋 `background-color` 和 `border-color`
...
- **WHEN** 多個元素使用相同的 transition
- **THEN** 應使用組合選擇器或定義可重用的 class
...
- **WHEN** 撰寫 media query 以支援響應式樣式
- **THEN** 應使用 `640px` (sm), `768px` (md), `1024px` (lg), `1280px` (xl)
- **THEN** 斷點值需遵循 Tailwind CSS 預設配置

#### Scenario: 斷點值應保持一致
- **WHEN** categories.css 和 tags.css 都需要 mobile 斷點
- **THEN** 兩者應使用相同的斷點值
...
- **WHEN** 檢查 article-cards.css 中的背景色使用次數
- **THEN** 若 `rgba(255, 255, 255, 0.65)` 使用超過 3 次
...
- **WHEN** 決定新增 CSS 變數的定義位置
- **THEN** 全域變數應在 `:root` 或 `00-design-tokens.css`
- **THEN** 模組專用變數可在該模組檔案頂部定義
...
#### Scenario: 移除中間的結構限定
- **WHEN** 簡化選擇器 `[data-taxonomy="categories"] section.flex.flex-wrap article`
- **THEN** 可改為 `[data-taxonomy="categories"] article`
- **THEN** 因為 `section.flex.flex-wrap` 不提供額外的必要限定

#### Scenario: 保留必要的上下文
- **WHEN** 需要區分不同上下文的相同元素而撰寫選擇器
- **THEN** 應保留足夠的上下文限定
...
- **WHEN** 查看 article-cards.css 中的 glass effect CSS
- **THEN** 應有註解說明：
...
#### Scenario: 選擇器策略註解
- **WHEN** 使用 data attribute 選擇器撰寫 CSS
...
- **WHEN** 評估 `a[href*="/categories/"]` 選擇器的可靠性
- **THEN** 應考慮是否有 edge cases (如外部連結包含 "/categories/")
- **THEN** 評估是否應改用 data attribute

#### Scenario: 考慮使用 data attribute 替代
- **WHEN** 需要區分 categories 和 tags badges
- **THEN** 可考慮在 badge partial 加入 `data-taxonomy-type`
- **THEN** 若確認無 edge cases 可保持現有 href 選擇器

#### Scenario: 文檔說明選擇器限制
- **WHEN** 使用 href 選擇器並撰寫註解
- **THEN** 應說明已知的限制和假設
...
- **WHEN** 評估 `max-width` (desktop-first) 的優化方式
